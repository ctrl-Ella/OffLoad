...
- **The one exception is what Mia says out loud, which is Spanish from Spain.** Her clips, the captions she hears, the prompts her two models are measured on. The screens are English — buttons, notices, error messages, her state labels — by team decision on 2026-09-20, which closed the question the earlier rule left open. `mia-voice` still governs how her spoken lines are written. So a component is named `PhoneSignIn`, its button says "Sign in with my phone", and when Mia proposes inside the call she says «¿Puedes ir tú?» while the buttons under her read "I'll go" and "I can't".
