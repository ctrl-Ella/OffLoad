...
**Nebius (Token Factory)** — two model tiers, both constrained to structured JSON-schema output, so a response that breaks the schema is impossible rather than merely unlikely. A small model extracts events and tasks out of a dictated sentence or a call's transcript; a larger one, the negotiator, decides who to ask and writes the proposal. The offload screen is where the small model runs today: a voice note goes through SLNG to a transcript, and `/api/structure-plan` turns that Spanish transcript into the titles, event-or-task label, and times the review screen shows — nothing more. Whether two of those times collide is never asked of the model; `conflicts.ts`, the same arithmetic the home screen and the clash screen already use, decides that from the timestamps the model extracted. Which model fills the tier came from measuring real candidates against a benchmark, not from picking a name off the catalogue: the intent classifier reaches 93.3% accuracy over 30 written cases, and the grounding check reaches 100% over 6 (`bench/results/`). That choice, made by measurement instead of by reading a catalogue, is the improvement this project can point to.

The 93.3% figure is over typed sentences, though, not spoken ones — the only way the product is actually used — so it was checked against the real path too: 29 of the 30 benchmark sentences read aloud, transcribed through the product's own `/api/transcribe` route (the thirtieth recording doesn't exist), and run through the same classifier. Accuracy drops to 86.2% over those 29 real transcripts. That is not a second improvement: it is a measured 7-point gap between a written sentence and what SLNG actually hands the model from a person's voice, written down rather than left for someone else to find. Both runs, same model, same day, are in `bench/results/`; the method and the one missing recording are documented in [`docs/specs/0004-nebius-benchmark-and-voice-measurement.md`](docs/specs/0004-nebius-benchmark-and-voice-measurement.md).
