  ...
  return (
    <main className="call-page">
      <div className="call-shell">
        ...
        <div className="call-proposal"><span className="proposal-mark"><Sparkles size={20} /></span><div><strong>Mia está {miaHasTurn ? "contigo" : "en silencio"}</strong><p>{miaHasTurn ? "Hablad de las opciones y confirmad con la persona que vaya a ayudar." : "Espera a que le deis la palabra para proponer una opción."}</p></div><button onClick={() => setMiaHasTurn(true)} disabled={miaHasTurn}>{miaHasTurn ? "Escuchando" : "Darle paso"}</button></div>
        {error && <p className="call-error" role="alert">{error}</p>}
        <div className="call-controls"><button onClick={toggleMicrophone} aria-label={microphoneOn ? "Silenciar micrófono" : "Activar micrófono"}>{microphoneOn ? <Mic size={21} /> : <MicOff size={21} />}</button><button onClick={toggleCamera} aria-label={cameraOn ? "Apagar cámara" : "Activar cámara"}>{cameraOn ? <Video size={21} /> : <VideoOff size={21} />}</button><button onClick={() => setConfirmOpen(true)} aria-label="Confirmar acuerdo"><Check size={21} /></button><button className="hangup" onClick={onExit} aria-label="Salir de la llamada"><PhoneOff size={21} /></button></div>
        <button className="call-finish" onClick={() => setConfirmOpen(true)}>Confirmar acuerdo y ver resumen</button>
      </div>
