...
**The demo week now carries its clash, and it is Sunday's.** Elvia leaves the Zona Franca office at 18:30 and the pool in Nou Barris starts at 19:00: thirty minutes for a trip estimated at thirty-five. Those are the two places `src/data/places.ts` was written around, and its comment says so. Until this was added, nothing in the week clashed — Thursday's work has no place on it, so no trip is estimated and nothing is claimed — and that had a consequence nobody could see from the room: without a clash the run never suspends, so every transcribed line came back "no day looked at" and Mia stayed at "Listening" whatever was said. Step 1 above still works for adding a second clash by hand.

**Its dates are fixed, so a demo on another day moves two rows.** Calculating them at seeding time would remove that, at the cost of a moving part in a file that is read at a glance and edited by hand. Weighed and left as it is.
...
**The support network is seeded now.** `npm run db:seed` writes Nicolás, Abuela Rosa and Vecina Marta with `circle = SUPPORT`, reading a number each from `TEST_PHONE_NICOLAS`, `TEST_PHONE_ROSA` and `TEST_PHONE_MARTA`. They carry no address: outside the core nobody connects a Google account. The negotiator reads them from the table, and `askWhetherToCall` drops its card when it finds none — so without these three rows Mia never asks whether to call anyone, which is what was happening.

**There is a button to ask Mia to step in, and it appears in one case only.** The room had none on purpose: Mia asks for the floor by herself, and a button standing there permanently shows an AI waiting to be invited. That reasoning holds while she can hear. It stops holding when no caption has arrived at all — waiting for her is then waiting for nothing, and the failure is invisible, since a room where captions never started looks exactly like a quiet one. So the room says "No captions yet" fifteen seconds in and offers the button beside it, and both disappear the moment a first caption lands. `POST /api/room/heard` already took `force`; what it gains is that a press carries neither a line nor a stream, so nothing of it reaches the negotiator as something someone said.

**Carlos needs Google connected for the proposal to name him.** The negotiator names someone in the core only when their calendar was read and is free in the slot. With his Google unconnected the run skips "¿Puedes ir tú?" and goes straight to the support network's card. The chain still ends where it should; what is lost is the beat where the partner answers first.
